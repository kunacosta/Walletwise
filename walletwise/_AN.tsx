...
import { chevronBackOutline, chevronForwardOutline, downloadOutline, trendingUpOutline, trendingDownOutline, pieChartOutline, flameOutline, moonOutline } from 'ionicons/icons';
...
export const Analytics: React.FC = () => {
  ...
  return (
    <IonPage>
      <PageHeader
        title="Analytics"
        subtitle="Narrate your money story"
        ...
        start={<ProBadge />}
          ...
          <>
            ...
            <IonButton fill="outline" onClick={exportCsv} aria-label="Export CSV"><IonIcon slot="icon-only" icon={downloadOutline} /></IonButton>
        ...
        <div className="section-stack">
          ...
          <IonCard aria-label="Expense distribution">
            <IonCardHeader className="section-title-row">
              <IonCardTitle>{drillCategory ? `Subcategories of ${drillCategory}` : 'Expense distribution'}</IonCardTitle>
              {drillCategory ? (
                <IonButton size="small" fill="outline" onClick={() => setDrillCategory(null)}>Back</IonButton>
              ) : null}
            </IonCardHeader>
            <IonCardContent>
              {total <= 0 ? (
                ...
                <div className="donut-layout">
                  <div className="donut-figure" role="img" aria-label="Expense donut chart">
                    <svg width={220} height={220}>
                      <title>Expense donut {drillCategory ? `for ${drillCategory}` : ''}</title>
                      ...
                      <g transform="translate(110,110)">
                        ...
                        {arcData.map((segment) => (
                          <circle
                            ...
                            role={!drillCategory ? 'button' : undefined}
                            tabIndex={!drillCategory ? 0 : -1}
                            aria-label={`${segment.label}: ${formatCurrency(segment.value)} (${segment.percent}%)`}
                            onClick={() => { if (!drillCategory) setDrillCategory(segment.label); }}
                            onKeyDown={(e) => {
                              if (!drillCategory && (e.key === 'Enter' || e.key === ' ')) {
                                e.preventDefault();
                                setDrillCategory(segment.label);
                              }
                            }}
                          />
                        ))}
                    ...
                    <div className="donut-center">
                      ...
                      <div className="donut-center__label">{drillCategory ? drillCategory : 'spent this month'}</div>
                    </div>
                  </div>
                    ...
                    {(drillCategory ? subData.map((s) => ({ label: s.subcategory, value: s.total })) : catData.map((c) => ({ label: c.category, value: c.total }))).map((row, i) => {
                      ...
                      return (
                        <IonItem
                          ...
                          button={!drillCategory}
                          detail={false}
                          onClick={() => { if (!drillCategory) setDrillCategory(row.label); }}
                        >
