...
const KeyButton: React.FC<{ label: string; onClick: () => void }> = ({ label, onClick }) => (
  <button type="button" className="add-txn-key" onClick={onClick}>
    {label}
  </button>
);
...
export const AddTransaction: React.FC = () => {
  ...
  const canClear = amountStr !== '0';
  const saveLabel = mode === 'transfer' ? 'Save Transfer' : `Save ${mode === 'income' ? 'Income' : 'Expense'}`;
    ...
    <IonPage>
      ...
      <IonContent className="ion-padding add-txn-content">
        ...
        <div className="add-txn-card add-txn-amount">
          ...
          <div
            className={`add-txn-amount-value add-txn-amount-${amountTone}`}
            role="status"
            aria-live="polite"
            aria-atomic="true"
            data-mode={mode}
          ...
          <IonNote className="add-txn-amount-mode" color={mode === 'income' ? 'success' : mode === 'expense' ? 'danger' : 'primary'}>
            {modeDescriptor}
          </IonNote>
        </div>
        ...
        <div className="add-txn-keypad">
          ...
          <div className="add-txn-actions">
            <IonButton expand="block" fill="outline" color="medium" onClick={clear} disabled={!canClear}>
              Clear
            </IonButton>
            <IonButton
              expand="block"
